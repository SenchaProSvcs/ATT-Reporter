...
module AttApi::Oauth
  ...
  ###
  def run_oauth_authorize
    @testName = "oauth_login"
    @startTestTime = Time.new
    @oauthCsv = [Time.now.to_f] #start_time
    
    # connect to oauth
    page = @agent.get "#{@baseURL}/oauth/authorize?scope=TL,DC,WAP,SMS,MMS,PAYMENT&client_id=#{@clientID}&redirect_uri=#{@localServer}/auth/callback"
