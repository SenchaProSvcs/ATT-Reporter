module AttApi::MMS
  ...
  def api_mms_send_json
    ...
    log "Sending MMS to #{@config['tel']}"
    
    url = "#{@config['api_host']}/1/messages/outbox/mms?access_token=#{@oauth_token}"
    
    log_error "Sending MMS to #{@config['tel']}"
    log_error url
  ...
  def api_mms_status
    ...
    url = "#{@config['api_host']}/1/messages/outbox/mms/#{@mms_id}?access_token=#{@oauth_token}"
