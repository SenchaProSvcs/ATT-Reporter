...
module AttApi::Payments
  ...
  def run_payment_new_transaction_consent(test_result)
    ...
    payload = {
      ...
      'MerchantPaymentRedirectUrl'=> "#{@config['local_server']}:#{@config['port']}/payment/callback"
    }
    run_notary_signed_payload(nil, payload)
    
    if @notary_signed_document.nil? or @notary_signature.nil?
      raise 'Error trying to sign payload'
    end
    
    test_result.url = "https://api.att.com/rest/3/Commerce/Payment/Transactions"
  ...
  def run_payment_new_subscription_consent(test_result)
    timestamp = Time.now.to_i.to_s
    payload = {
      "Amount" => 0.99,
      "Category" => 1,
    ...
    test_result.url = "https://api.att.com/rest/3/Commerce/Payment/Subscriptions"
    test_result.verb= "GET"
    test_result.data= {
      "clientid"              => "{client_id}",
      "Signature"             => @notary_signature,
  ...
  def api_payment_new(format)
    ...
    begin
      ...
      newPaymentRequestData = {
        ...
        :merchantCancelRedirectUrl => "#{@config['local_server']}:#{@config['port']}/payments/cancel",
        :merchantFulfillmentRedirectUrl => "#{@config['local_server']}:#{@config['port']}/payments/deliver",
      ...
      url = "#{@config['api_host']}/1/payments/transactions?access_token=#{@oauth_token}"
  ...
  def api_payment_status
    ...
    url = "#{@config['api_host']}/1/payments/transactions/#{@new_payment_txid}?access_token=#{@oauth_token}"
    ...
    begin
      ...
      if paymentStatus['transactionStatus'] == 'SUCCESSFUL'
        ...
        begin
          ...
          url = "#{@config['api_host']}/1/payments/transactions/#{transaction_id}?action=commit&access_token=#{@oauth_token}"
  ...
  def api_payment_refund
    ...
    url = "#{@config['api_host']}/1/payments/transactions/#{@new_payment_txid}?access_token=#{@oauth_token}&action=refund"
