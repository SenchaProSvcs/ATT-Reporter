...
# See README.md
#
# Change the variables below and run `ruby test.rb`
#    or from the server run 'nohup ruby test.rb &'
...
# get the config, the default is att-api.properties or you can pass in the filename of a different
# config file.  note that the assumption is that the config file will be on the conf folder
...
def setup
  ...
  $reporter = AttApiReporter.new(
    ...
    :debug => AttApiReporter::DEBUG_INFO, # we actually want DEBUG_INFO so the UI will have all of the data
...
Thread.new do
  ...
  while true
    ...
    # puts "tests started, next run will be in 60 minutes"
    sleep 3600
    puts ""
  end
...
######## Below are local Sinatra endpoints to deal with API callbacks ########

###
# Handle auth callback. Pass control back to the reporter.
##
get '/auth/callback' do
  $reporter.getToken(params[:code])
end

###
# Handle payment notifications
##
get '/payments/notify' do
  $reporter.payment_notification(params)
end
...
get '/status' do
  testResults =  $reporter.test_results()
  testNames =  $reporter.test_names()
  haml :status, :locals => { :testsStartTime =>  $testsStartTime.strftime("%m/%d/%Y %R:%S"), :testsAreRunning => $testsAreRunning, :baseURL => $reporter.base_url, :testUser => $reporter.user_id, :tests => tests, :issues => issues, :testNames => testNames, :testResults => testResults, :reporter => $reporter}
end
