module AttApi::SMS
  ...
  def api_v2sms_send_form
    log "Sending SMS to #{@tel}"
    data =  { :Address => "tel:#{@tel}", :Message => 'Works from SENCHA 2'};
    url = "#{@baseURL}/rest/sms/2/messaging/outbox?access_token=#{@oauth_token}"
  ...
  def api_v2sms_send_json
    ...
    data = "{'Address' => 'tel:#{@tel}', 'Message' => 'Works from SENCHA api v2'}";
    url = "#{@baseURL}/rest/sms/2/messaging/outbox?access_token=#{@oauth_token}"
    log_error "Sending SMS to #{data}"
    log_error url
...
#
    ...
    url = "#{@baseURL}/rest/sms/2/messaging/outbox/#{@sms_id}?access_token=#{@oauth_token}"
  ...
  def api_v2sms_receive
    ...
    url = "#{@baseURL}/2/messages/inbox/sms?access_token=#{@oauth_token}&registrationID=#{@shortCode}&format=json"
